...
if(in_array($domain, $disposable_list)){ 
    ...
    if ($nbRow==0) {
        ...
        $sql = "INSERT INTO `user` (`email`,`password`,`name`,`token`,`expirationToken`,`creationDate`,`ipAdress`) values (?,?,?,?,?,?,?)";
        $stmt = $con->prepare($sql);
        $stmt->bind_param('sssssss',$mail, $passwordHash, $name, $user_key, $expirationTokenDate, $creationDate, $ipUser);
        $stmt->execute();
